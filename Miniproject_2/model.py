...
class Model(Module):
    def __init__(self):
        ...
        self.model = Sequential(Conv2d(3, 100, kernel_size=2, stride=2), 
                                ReLU(),
                                Conv2d(100, 100, kernel_size=2, stride=2),
                                ReLU(),
                                NearestUpsampling(scale_factor=2),
                                Conv2d(100, 100, kernel_size=3, stride=1, padding=1),
                                ReLU(),
                                NearestUpsampling(scale_factor=2),
                                Conv2d(100, 3, kernel_size=3, stride=1, padding=1),
                                Sigmoid())
        
        self.optimizer = SGD(self.model, lr=1e-1)
        self.criterion = MSE()
